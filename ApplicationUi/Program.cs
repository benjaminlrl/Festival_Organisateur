    ...
    internal static class Program
    {
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                // -------------------------------------------------------
                // 9. JEUX  (FK → Plateforme via table de jointure)
                // -------------------------------------------------------
                if (!context.Jeux.Any())
                {
                    var plateformes = context.Plateformes.ToList();
                    ...
                    context.Jeux.AddRange(new List<Jeu>
                    {
                        ...
                        // Super Smash Bros. Ultimate → Nintendo Switch / Switch 2 UNIQUEMENT (pas de PS5 !)
                ...
                // -------------------------------------------------------
                // 11. SOUMIS VOTES  (FK → Jeu + Plateforme via jointure)
                //     Toutes les combinaisons (Jeu, Plateforme) vérifiées
                //     contre les définitions de jeux ci-dessus.
                // -------------------------------------------------------
                if (!context.SoumisVotes.Any())
                {
                    // On recharge les jeux AVEC leurs plateformes depuis la base
                    var jeux = context.Jeux.Include(j => j.Plateformes).ToList();
                    var plateformes = context.Plateformes.ToList();

                    Jeu J(string titre) => jeux.First(j => j.Titre == titre);
                    Plateforme P(string lib) => plateformes.First(p => p.Libelle == lib);

                    // Helper pour récupérer l'IdPlateforme d'un jeu en vérifiant qu'elle existe bien
                    int PourJeu(Jeu jeu, string libPlateforme)
                    {
                        var plat = jeu.Plateformes.FirstOrDefault(p => p.Libelle == libPlateforme)
                            ?? throw new Exception($"Le jeu '{jeu.Titre}' n'existe pas sur '{libPlateforme}' !");
                        return plat.IdPlateforme;
                    }

                    var jeuMK = J("Mario Kart 8 Deluxe");
                    var jeuFtn = J("Fortnite");
                    var jeuHalo = J("Halo Infinite");
                    var jeuVal = J("Valorant");
                    var jeuLoL = J("League of Legends");
                    var jeuCS2 = J("Counter-Strike 2");
                    var jeuFC25 = J("EA Sports FC 25");
                    var jeuElden = J("Elden Ring");
                    var jeuMC = J("Minecraft");
                    var jeuSmash = J("Super Smash Bros. Ultimate");

                    context.SoumisVotes.AddRange(new List<SoumisVote>
                    {
                        new SoumisVote { DateDebutVote = DateTime.Now.AddDays(-5), DateFinVote = DateTime.Now.AddDays(15), IdJeu = jeuMK.IdJeu,    IdPlateforme = PourJeu(jeuMK,    "Nintendo Switch")   },
                        new SoumisVote { DateDebutVote = DateTime.Now.AddDays(-5), DateFinVote = DateTime.Now.AddDays(15), IdJeu = jeuFtn.IdJeu,   IdPlateforme = PourJeu(jeuFtn,   "PC (Windows)")      },
                        new SoumisVote { DateDebutVote = DateTime.Now.AddDays(-5), DateFinVote = DateTime.Now.AddDays(15), IdJeu = jeuHalo.IdJeu,  IdPlateforme = PourJeu(jeuHalo,  "Xbox Series X/S")   },
                        new SoumisVote { DateDebutVote = DateTime.Now.AddDays(-5), DateFinVote = DateTime.Now.AddDays(15), IdJeu = jeuVal.IdJeu,   IdPlateforme = PourJeu(jeuVal,   "PC (Windows)")      },
                        new SoumisVote { DateDebutVote = DateTime.Now.AddDays(-5), DateFinVote = DateTime.Now.AddDays(15), IdJeu = jeuLoL.IdJeu,   IdPlateforme = PourJeu(jeuLoL,   "PC (Windows)")      },
                        new SoumisVote { DateDebutVote = DateTime.Now.AddDays(-5), DateFinVote = DateTime.Now.AddDays(15), IdJeu = jeuCS2.IdJeu,   IdPlateforme = PourJeu(jeuCS2,   "PC (Windows)")      },
                        new SoumisVote { DateDebutVote = DateTime.Now.AddDays(-5), DateFinVote = DateTime.Now.AddDays(15), IdJeu = jeuFC25.IdJeu,  IdPlateforme = PourJeu(jeuFC25,  "PlayStation 5")     },
                        new SoumisVote { DateDebutVote = DateTime.Now.AddDays(-5), DateFinVote = DateTime.Now.AddDays(15), IdJeu = jeuElden.IdJeu, IdPlateforme = PourJeu(jeuElden, "PlayStation 5")     },
                        new SoumisVote { DateDebutVote = DateTime.Now.AddDays(-5), DateFinVote = DateTime.Now.AddDays(15), IdJeu = jeuMC.IdJeu,    IdPlateforme = PourJeu(jeuMC,    "PlayStation 5")     },
                        new SoumisVote { DateDebutVote = DateTime.Now.AddDays(-5), DateFinVote = DateTime.Now.AddDays(15), IdJeu = jeuSmash.IdJeu, IdPlateforme = PourJeu(jeuSmash, "Nintendo Switch")   },
                    });
                    context.SaveChanges();

                    // Voter : on récupère les SoumisVotes persistés pour avoir les bons IDs
                    SoumisVote SV(int idJeu, int idPlat) =>
                        context.SoumisVotes.First(s => s.IdJeu == idJeu && s.IdPlateforme == idPlat);

                    context.Voter.AddRange(new List<Voter>
                    {
                        // Mario Kart → Nintendo Switch
                        new Voter { IdJeu = jeuMK.IdJeu,    IdPlateforme = PourJeu(jeuMK,    "Nintendo Switch"), IdUser = 946, DateVote = DateTime.Now.AddDays(-3) },
                        new Voter { IdJeu = jeuMK.IdJeu,    IdPlateforme = PourJeu(jeuMK,    "Nintendo Switch"), IdUser = 785, DateVote = DateTime.Now.AddDays(-1) },
                        new Voter { IdJeu = jeuMK.IdJeu,    IdPlateforme = PourJeu(jeuMK,    "Nintendo Switch"), IdUser = 453, DateVote = DateTime.Now             },
                        new Voter { IdJeu = jeuMK.IdJeu,    IdPlateforme = PourJeu(jeuMK,    "Nintendo Switch"), IdUser = 864, DateVote = DateTime.Now.AddDays(-4) },
                        new Voter { IdJeu = jeuMK.IdJeu,    IdPlateforme = PourJeu(jeuMK,    "Nintendo Switch"), IdUser = 125, DateVote = DateTime.Now.AddDays(-3) },
                        new Voter { IdJeu = jeuMK.IdJeu,    IdPlateforme = PourJeu(jeuMK,    "Nintendo Switch"), IdUser = 666, DateVote = DateTime.Now.AddDays(-2) },
                        // Fortnite → PC
                        new Voter { IdJeu = jeuFtn.IdJeu,   IdPlateforme = PourJeu(jeuFtn,   "PC (Windows)"),    IdUser = 666, DateVote = DateTime.Now.AddDays(-2) },
                        new Voter { IdJeu = jeuFtn.IdJeu,   IdPlateforme = PourJeu(jeuFtn,   "PC (Windows)"),    IdUser = 453, DateVote = DateTime.Now.AddDays(-1) },
                        new Voter { IdJeu = jeuFtn.IdJeu,   IdPlateforme = PourJeu(jeuFtn,   "PC (Windows)"),    IdUser = 125, DateVote = DateTime.Now.AddDays(-1) },
                        new Voter { IdJeu = jeuFtn.IdJeu,   IdPlateforme = PourJeu(jeuFtn,   "PC (Windows)"),    IdUser = 785, DateVote = DateTime.Now.AddDays(-4) },
                        new Voter { IdJeu = jeuFtn.IdJeu,   IdPlateforme = PourJeu(jeuFtn,   "PC (Windows)"),    IdUser = 946, DateVote = DateTime.Now             },
                        // Halo → Xbox Series X/S
                        new Voter { IdJeu = jeuHalo.IdJeu,  IdPlateforme = PourJeu(jeuHalo,  "Xbox Series X/S"), IdUser = 163, DateVote = DateTime.Now.AddDays(-3) },
                        new Voter { IdJeu = jeuHalo.IdJeu,  IdPlateforme = PourJeu(jeuHalo,  "Xbox Series X/S"), IdUser = 453, DateVote = DateTime.Now.AddDays(-3) },
                        new Voter { IdJeu = jeuHalo.IdJeu,  IdPlateforme = PourJeu(jeuHalo,  "Xbox Series X/S"), IdUser = 785, DateVote = DateTime.Now.AddDays(-3) },
                        // Valorant → PC
                        new Voter { IdJeu = jeuVal.IdJeu,   IdPlateforme = PourJeu(jeuVal,   "PC (Windows)"),    IdUser = 125, DateVote = DateTime.Now.AddDays(-5) },
                        // League of Legends → PC
                        new Voter { IdJeu = jeuLoL.IdJeu,   IdPlateforme = PourJeu(jeuLoL,   "PC (Windows)"),    IdUser = 942, DateVote = DateTime.Now.AddDays(-5) },
                        new Voter { IdJeu = jeuLoL.IdJeu,   IdPlateforme = PourJeu(jeuLoL,   "PC (Windows)"),    IdUser = 457, DateVote = DateTime.Now.AddDays(-3) },
                        new Voter { IdJeu = jeuLoL.IdJeu,   IdPlateforme = PourJeu(jeuLoL,   "PC (Windows)"),    IdUser = 453, DateVote = DateTime.Now.AddDays(-4) },
                        new Voter { IdJeu = jeuLoL.IdJeu,   IdPlateforme = PourJeu(jeuLoL,   "PC (Windows)"),    IdUser = 125, DateVote = DateTime.Now.AddDays(-5) },
                        // Counter-Strike 2 → PC
                        new Voter { IdJeu = jeuCS2.IdJeu,   IdPlateforme = PourJeu(jeuCS2,   "PC (Windows)"),    IdUser = 163, DateVote = DateTime.Now             },
                        // EA Sports FC 25 → PS5
                        new Voter { IdJeu = jeuFC25.IdJeu,  IdPlateforme = PourJeu(jeuFC25,  "PlayStation 5"),   IdUser = 180, DateVote = DateTime.Now             },
                        new Voter { IdJeu = jeuFC25.IdJeu,  IdPlateforme = PourJeu(jeuFC25,  "PlayStation 5"),   IdUser = 457, DateVote = DateTime.Now             },
                        new Voter { IdJeu = jeuFC25.IdJeu,  IdPlateforme = PourJeu(jeuFC25,  "PlayStation 5"),   IdUser = 917, DateVote = DateTime.Now.AddDays(-1) },
                        new Voter { IdJeu = jeuFC25.IdJeu,  IdPlateforme = PourJeu(jeuFC25,  "PlayStation 5"),   IdUser = 125, DateVote = DateTime.Now             },
                        new Voter { IdJeu = jeuFC25.IdJeu,  IdPlateforme = PourJeu(jeuFC25,  "PlayStation 5"),   IdUser = 74,  DateVote = DateTime.Now             },
                        new Voter { IdJeu = jeuFC25.IdJeu,  IdPlateforme = PourJeu(jeuFC25,  "PlayStation 5"),   IdUser = 163, DateVote = DateTime.Now.AddDays(-2) },
                        new Voter { IdJeu = jeuFC25.IdJeu,  IdPlateforme = PourJeu(jeuFC25,  "PlayStation 5"),   IdUser = 453, DateVote = DateTime.Now             },
                        // Minecraft → PS5
                        new Voter { IdJeu = jeuMC.IdJeu,    IdPlateforme = PourJeu(jeuMC,    "PlayStation 5"),   IdUser = 453, DateVote = DateTime.Now             },
                        // Elden Ring → PS5
                        new Voter { IdJeu = jeuElden.IdJeu, IdPlateforme = PourJeu(jeuElden, "PlayStation 5"),   IdUser = 453, DateVote = DateTime.Now.AddDays(-1) },
                        new Voter { IdJeu = jeuElden.IdJeu, IdPlateforme = PourJeu(jeuElden, "PlayStation 5"),   IdUser = 845, DateVote = DateTime.Now.AddDays(-3) },
                        new Voter { IdJeu = jeuElden.IdJeu, IdPlateforme = PourJeu(jeuElden, "PlayStation 5"),   IdUser = 920, DateVote = DateTime.Now             },
                        new Voter { IdJeu = jeuElden.IdJeu, IdPlateforme = PourJeu(jeuElden, "PlayStation 5"),   IdUser = 841, DateVote = DateTime.Now.AddDays(-2) },
                        new Voter { IdJeu = jeuElden.IdJeu, IdPlateforme = PourJeu(jeuElden, "PlayStation 5"),   IdUser = 120, DateVote = DateTime.Now             },
                        // Super Smash Bros. → Nintendo Switch
                        new Voter { IdJeu = jeuSmash.IdJeu, IdPlateforme = PourJeu(jeuSmash, "Nintendo Switch"), IdUser = 510, DateVote = DateTime.Now             },
                    });
                    context.SaveChanges();
                }
